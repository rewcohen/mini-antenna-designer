...
<svg width="{width:.1f}" height="{height:.1f}" viewBox="0 0 {width:.1f} {height:.1f}" xmlns="http://www.w3.org/2000/svg">
  <title>PCB Antenna Design - Laser Etching Ready</title>
  <desc>{self._svg_description(metadata)}</desc>
  ...
  {feed_svg}
    ...
    def _svg_description(self, metadata: Optional[Dict]) -> str:
        """Accurate SVG description from metadata (no hardcoded antenna type)."""
        md = metadata or {}
        parts = []
        band = md.get('band_name')
        dtype = md.get('design_type')
        if band:
            parts.append(str(band))
        if dtype:
            parts.append(str(dtype).replace('_', ' '))
        label = ' - '.join(parts) if parts else 'Planar antenna'
        return f"{label} - generated by Mini Antenna Designer"
    ...
    def _generate_svg_annotations(self, wire_segments: List[tuple], transform_func,
        ...
        try:
            ...
            title_x = 20
            title_y = 30
            annotations.append(f'<text x="{title_x}" y="{title_y}" class="title-text">PCB ANTENNA</text>')
